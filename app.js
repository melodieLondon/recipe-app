...
async function loadRecipes(force) {
  ...
  // Use cached recipes unless forced refresh
  if (!force) {
  ...
  try {
    // Add cache-buster when forcing refresh
    const url = force
...
// ── CSV parser ───────────────────────────────────────────────
// Expected columns: category | name | subtitle | meta | ingredients | method | nutrition
...
function nextRecipe() {
  if (currentCat) pickCategory(currentCat);
}
...
function renderRecipe(r) {
  ...
  const ingHtml = r.ingredients
    .map(i => `<li class="ing-item">${i}</li>`)
    .join("");

  const nutHtml = r.nutrition
    ? `<div class="nutrition-label">${r.nutrition}</div>`
    : "";

  document.getElementById("recipe-content").innerHTML = `
    <div class="recipe-card" style="--accent:${meta.accent}">
      <div class="recipe-cat-pill">
